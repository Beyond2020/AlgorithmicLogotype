...
////3Dバージョンのため、カーソルで視点を変えてみることができます。
...
void ofApp::setup(){
    ...
    for(int j=0;j<1200;j++){
        //円の解像度
        phase += 0.00524;
        vec.x = cos(phase) * radius;
        vec.y = sin(phase) * radius;
        mesh.addVertex(ofVec3f(vec.x,vec.y,0));
    }
}
